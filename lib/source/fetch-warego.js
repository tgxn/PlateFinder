...
async function loadData() {
  ...
  if (!htmlData) {
    ...
    // dump to local file
    fs.writeFileSync("lib/data/warego.orig.html", htmlData);
...
function toCleanTable(table, typeInt) {
  const arrays = table.map((row) => {
    // for the first element (code) - remove spaces and special chars in the code + convert to uppercase
    const cleanCode = row[0].replace(/\s/g, "").toUpperCase();

    // just trim the supposed name for the entry
    ...
    if (cleanCode == "SRL/SSRL") {
      console.debug("applying datamod", cleanCode);
...
async function fetchWaregoInfo() {
  ...
  for (const [tableIndex, table] of tables.entries()) {
    ...
    console.log(
      `found table with ${parsedTable.length} items, first item:`,
      parsedTable[0],
    );
